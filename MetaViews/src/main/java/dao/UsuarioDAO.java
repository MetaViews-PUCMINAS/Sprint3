...
public class UsuarioDAO {
	...
	public boolean inserirUsuario(Usuario usuario) {
		...
		try {  
			...
			st.executeUpdate("INSERT INTO Usuario (idUsuario,email, idade, senha, nome) "
					       + "VALUES ("+usuario.getId()+ ", '" +usuario.getEmail()+ ", '" + usuario.getIdade() + "', '"  
	...
	public boolean atualizarUsuario(Usuario usuario) {
		...
		try {  
			Statement st = conexao.createStatement();
			String sql = "UPDATE Usuario SET email = '" +usuario.getId()+ ", '" + usuario.getEmail() + "', senha = '"  
			...
			ResultSet rs = st.executeQuery("SELECT * FROM usuario");		
	         if(rs.next()){
	             ...
	             for(int i = 0; rs.next(); i++) {
	                usuarios[i] = new Usuario(rs.getInt("idUsuario"), rs.getString("email"),rs.getInt("idade"), 
